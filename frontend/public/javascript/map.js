import { test } from './testrequire.js';

require(["esri/views/MapView", "esri/Map", "esri/widgets/Sketch/SketchViewModel", "esri/Graphic", "esri/layers/GraphicsLayer", "dojo/domReady!javascript/map", "esri/geometry/Polyline"], function (MapView, Map, SketchViewModel, Graphic, GraphicsLayer) {
  let editGraphic;
  let minimalRadius = 1000.0; // GraphicsLayer to hold graphics created via sketch view model
  ...
  view.when(function () {
    ...
    setUpClickHandler(); // Listen to create-complete event to add a newly created graphic to view

    sketchViewModel.on("create-complete", addGraphic); // Listen the sketchViewModel's update-complete and update-cancel events

    sketchViewModel.on("update-complete", updateGraphic);
    sketchViewModel.on("update-cancel", updateGraphic); // called when sketchViewModel's create-complete event is fired.
    ...
    function updateGraphic(event) {
      ...
      graphicsLayer.add(graphic); // set the editGraphic to null update is complete or cancelled.
          ...
          if (results.length > 0) {
            for (var i = 0; i < results.length; i++) {
              ...
              if (!editGraphic && results[i].graphic.layer.id === "tempGraphics") {
                ...
                editGraphic = results[i].graphic; // Remove the graphic from the GraphicsLayer
