require(["esri/Map", "esri/views/MapView", "esri/views/2d/draw/Draw", "esri/Graphic", "esri/geometry/geometryEngine"], function (Map, MapView, Draw, Graphic, geometryEngine) {
  ...
  document.getElementById("line-button").onclick = function () {
    view.graphics.removeAll(); // creates and returns an instance of PolyLineDrawAction

    const action = draw.create("polyline"); // focus the view to activate keyboard shortcuts for sketching

    view.focus(); // listen polylineDrawAction events to give immediate visual feedback
  ...
  function updateVertices(event) {
    ...
    const result = createGraphic(event); // if the last vertex is making the line intersects itself,
  ...
  function createGraphic(event) {
    ...
    const intersectingSegment = getIntersectingSegment(graphic.geometry); // Add a new graphic for the intersecting segment.
  ...
  function isSelfIntersecting(polyline) {
    ...
    const line = polyline.clone(); //get the last segment from the polyline that is being drawn
